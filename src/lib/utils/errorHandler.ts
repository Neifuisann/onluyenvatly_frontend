...
const ERROR_MESSAGES: Record<string, string> = {
  // Registration errors
  'DUPLICATE_PHONE': 'Số điện thoại này đã được đăng ký. Vui lòng sử dụng số điện thoại khác.',
  ...
  'ACCOUNT_NOT_FOUND': 'Tài khoản không tồn tại. Vui lòng kiểm tra lại số điện thoại.',
  'INCORRECT_PASSWORD': 'Mật khẩu không chính xác. Vui lòng thử lại.',
  'ACCOUNT_PENDING_APPROVAL': 'Tài khoản của bạn đang chờ được giáo viên phê duyệt.',
  'DEVICE_MISMATCH': 'Bạn chỉ có thể đăng nhập từ thiết bị đã đăng ký. Liên hệ giáo viên để được hỗ trợ.',
  ...
  if (error instanceof Error && 'isAxiosError' in error) {
    ...
    if (axiosError.response?.data) {
      ...
      if (data.error && typeof data.error === 'string') {
        ...
        if (data.error !== 'Internal server error' && data.error !== 'INTERNAL_ERROR') {
          return data.error;
        }
      }
